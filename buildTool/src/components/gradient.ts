...
function chooseGradientColors(palette: GradientPalette): [string, string] {
	...
	const startBase =
		startBaseColors[Math.floor(Math.random() * startBaseColors.length)]
	const endBase =
		endBaseColors[Math.floor(Math.random() * endBaseColors.length)]
