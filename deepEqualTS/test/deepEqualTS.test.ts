...
import { DeepEqualEngine } from '../src/DeepEqualEngine';
...
describe('DeepEqualEngine', () => {
  ...
  describe('Массивы', () => {
    ...
    test('массивы упорядоченные/не упорядоченные', () => {
      expect(engine.deepEqual([1, 2, 3], [1, 3, 2], { ignoreArrayOrder: true })).toBe(true);
      expect(engine.deepEqual([1, 2, 3], [1, 3, 2], { ignoreArrayOrder: false })).toBe(false);
    });
  });
  ...
  describe('Объекты', () => {
    ...
    test('объекты с одинаковыми ключами ', () => {
      ...
      expect(engine.deepEqual(obj2, obj3)).toBe(false);
    });
  ...
  describe('Циклические ссылки', () => {
    test('объект ссылается сам на себя', () => {
      const obj: { name: string; self?: unknown } = { name: 'cycle' };
      obj.self = obj;
      expect(engine.deepEqual(obj, obj)).toBe(true);
    });
    
    test('разные объекты', () => {
      const obj1: { name: string; self?: unknown } = { name: 'cycle1' };
      const obj2: { name: string; self?: unknown } = { name: 'cycle2' };
      obj1.self = obj1;
      obj2.self = obj2;
      expect(engine.deepEqual(obj1, obj2)).toBe(false);
    });
  ...
  describe(' Различные типы', () => {
    test('массив и string', () => {
      expect(engine.deepEqual([1, 2, 3, 4], 'hello')).toBe(false);
    });
    
    test('объект и number', () => {
      expect(engine.deepEqual({ name: 'Alexandra', age: 30 }, 30)).toBe(false);
    });
  });
